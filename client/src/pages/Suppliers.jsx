...
const Suppliers = () => {
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-20 max-w-[1600px] mx-auto">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                {isOwner && (
                    <button
                        ...
                        className="btn bg-pharmacy-900 text-white flex items-center justify-center gap-3 py-4 px-10 rounded-[20px] shadow-2xl shadow-pharmacy-200 font-black uppercase tracking-widest text-[10px] active:scale-95 transition-all"
                    >
                        <Plus size={20} />
                        Register Provider
                    </button>
            ...
            <div className="bg-white p-4 lg:p-6 rounded-[32px] shadow-sm border border-pharmacy-50 flex flex-col lg:flex-row gap-4 lg:items-center justify-between relative overflow-hidden group">
                <div className="absolute inset-y-0 left-0 w-1.5 bg-primary-500 rounded-full group-focus-within:h-full h-12 transition-all duration-300 my-auto ml-2"></div>
                <div className="relative w-full lg:w-[450px] ml-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                {loading ? (
                    [1, 2, 3, 4, 5, 6].map(i => (
                        <div key={i} className="h-72 bg-white rounded-[40px] animate-pulse border border-pharmacy-50 shadow-sm relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-pharmacy-50 rounded-full -mr-16 -mt-16 opacity-40"></div>
                        </div>
                    ))
                ) : filteredSuppliers.map((sup) => (
                    <div key={sup._id} className="bg-white p-8 lg:p-10 rounded-[48px] border border-pharmacy-100 shadow-sm hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] transition-all group relative overflow-hidden active:scale-[0.98]">
